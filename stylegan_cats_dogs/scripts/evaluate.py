"""
ResNet50 species-score evaluation for StyleGAN2-ADA cats+dogs models.

For each network-snapshot-*.pkl (kimg timestamp):
  - generate 90 images in memory (not saved)
...
STYLEGAN_ROOT = SCRIPT_DIR.parent
REPO_ROOT = STYLEGAN_ROOT.parent
...
# ImageNet-1k (ResNet50): dog breeds 151–268, cat breeds 281–285
...
MODELS_DIR = STYLEGAN_ROOT / "models"
OUTPUTS_DIR = STYLEGAN_ROOT / "outputs_cats_dogs"

RUN_DIR = REPO_ROOT / "stylegan_runs_cats_dogs" / (
    "00000-cats_dogs128-mirror-auto1-kimg1000-noaug"
)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Cat/dog ResNet50 scores for StyleGAN cats+dogs checkpoints"
        )
    )
    parser.add_argument(
        "--truncation-psi",
